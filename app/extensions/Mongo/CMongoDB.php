 ...
 */
class CMongoDB extends CApplicationComponent {
    /**
	 ...
	 * effective when the CMongoDB object is used as an application component.
	 */
	...
	public function connect(){
		if($this->_mongo===null){
			try	{
				Yii::trace('Opening MongoDB connection', 'ext.MongoDb.CMongoDB');
				...
				throw new CMongoDBException(Yii::t(
                        'yii',
                        'CMongoDB failed to open connection: {error}',
	...
	protected function close(){
        if($this->_mongo!==null){
            $this->_mongo->close();
            $this->_mongo=null;
            Yii::trace('Closing MongoDB connection', 'ext.MongoDb.CMongoDB');
        }
	}
	...
	public function getMongoInstance(){
        if($this->_mongo===null){
            if(empty($this->connectionString)){
				throw new CMongoDBException(Yii::t('mongo', 'CMongoDB.connectionString cannot be empty.'));
            }
     ...
     */
    public function getCollection($collectionName, $className = 'CMongoCollection'){
        if(!isset($this->_collections[$collectionName])){
            if(empty($this->db)){
                throw new CMongoException(Yii::t('mongo', 'CMongoDB.db can not be empty.'));
            }
    ...
    public function getRawCollection($collectionName){
        try {
            ...
            throw new CMongoException(Yii::t('mongo', 'CMongoDB.db is invalid.'));
        }
    }
